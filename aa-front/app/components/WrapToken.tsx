...
export const WrapToken = ({
  ...
  const { result: txStatus, setSuccess, setError, clearResult } = useTransactionResult()
  ...
  const handleWrap = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(result.txHash, `Successfully wrapped ${wrapAmount} ETH to WSEP`)
        setWrapAmount('')
  ...
  const handleUnwrap = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(result.txHash, `Successfully unwrapped ${unwrapAmount} WSEP to ETH`)
        setUnwrapAmount('')
  ...
  return (
    <Card className="border-slate-200 shadow-sm overflow-hidden">
      ...
      <CardContent className="p-6 space-y-4">
        {txStatus && (
          <Alert
            className={`${txStatus.success ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}
          >
            <div className="flex items-start gap-2">
              ...
              <AlertDescription>{txStatus.message || txStatus.error}</AlertDescription>
            </div>
          </Alert>
        )}
