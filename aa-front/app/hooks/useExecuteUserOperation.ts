...
import { buildEip712Domain, eip712Types, toPackedUserOperation, HEX_EMPTY } from '../utils/aaV08'
...
export function useExecuteUserOperation() {
  ...
  const executeBatch = useCallback(
    async (userOps: UserOperationV08[]) => {
      ...
      for (const raw of userOps) {
        ...
        let reGas: Partial<
          Pick<UserOperationV08, 'callGasLimit' | 'verificationGasLimit' | 'preVerificationGas'>
        > = {}
        try {
  ...
  const execute = useCallback(
    async (userOperation: UserOperationV08) => {
